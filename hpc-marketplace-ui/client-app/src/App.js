...
export default function App() {
  ...
  // New state for all requests & bids
  ...
  // ---- Accept a bid (works in both “bids” and “all_bids” views) ----
  const acceptBid = async (bid, backTo='status',reqId) => {
    console.log('Accepting bid:', bid);
    await post(`/clients/${clientId}/requests/${bid.request_id || reqId}/bids/${bid.id}/accept`, {});
    setView(backTo);
  };
    ...
    <div className="app-container">
      ...
      {view === 'login' && (
        <section>
          ...
          />
          <button className="submit" onClick={login} disabled={!clientId}>Login</button>
        </section>
      )}
      ...
      {view === 'bids' && (
        <section>
          ...
          {bids.length===0 ? <p>No bids yet…</p> : bids.map(b => (
            <div key={b.id}>
              Provider {b.provider_id}: ${b.price.toFixed(2)}
              <button style={{marginLeft:'1rem'}} onClick={()=>acceptBid({...b},'status',reqId)}>
                Accept
              </button>
            </div>
          ))}
        ...
        <section>
          ...
          {requests.map(r => (
            <div key={r.id}>
              ...
              {r.result_output && <pre>{r.result_output}</pre>}
            </div>
          ))}
        </section>
      )}
        ...
        <section>
          ...
          {allRequests.map(r => (
            <div key={r.id} style={{marginBottom:'1rem'}}>
              <strong>Request #{r.id}</strong> — {r.status}
              <button style={{marginLeft:'1rem'}} onClick={()=>viewBids(r.id)}>
                View Bids
              </button>
